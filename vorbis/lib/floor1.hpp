...
int floor1_encode(oggpack_buffer *opb,vorbis_block *vb,
  ...
  if(post){
    ...
    for(i=0,j=2;i<info->partitions;i++){
      int this_class=info->partitionclass[i];
      int cdim=info->class_dim[this_class];
      int csubbits=info->class_subs[this_class];
      int csub=1<<csubbits;
      ...
      if(csubbits){
        int maxval[8];
        for(k=0;k<csub;k++){
          int booknum=info->class_subbook[this_class][k];
          if(booknum<0){
            maxval[k]=1;
          }else{
            maxval[k]=sbooks[info->class_subbook[this_class][k]]->entries;
          }
        }
        for(k=0;k<cdim;k++){
          for(l=0;l<csub;l++){
        ...
        /* write it */
        look->phrasebits+=
          vorbis_book_encode(books+info->class_book[this_class],cval,opb);
...
#ifdef TRAIN_FLOOR1
        {
          ...
          sprintf(buffer,"line_%dx%ld_class%d.vqd",
                  vb->pcmend/2,posts-2,this_class);
          of=fopen(buffer,"a");
          fprintf(of,"%d\n",cval);
          fclose(of);
        }
      ...
      /* write post values */
      for(k=0;k<cdim;k++){
        int book=info->class_subbook[this_class][bookas[k]];
        if(book>=0){
...
#ifdef TRAIN_FLOOR1
          {
            ...
            sprintf(buffer,"line_%dx%ld_%dsub%d.vqd",
                    vb->pcmend/2,posts-2,this_class,bookas[k]);
            of=fopen(buffer,"a");
            fprintf(of,"%d\n",out[j+k]);
            fclose(of);
          }
...
static void *floor1_inverse1(vorbis_block *vb,vorbis_look_floor *in){
  ...
  if(oggpack_read(&vb->opb,1)==1){
    ...
    for(i=0,j=2;i<info->partitions;i++){
      ...
      if(csubbits){
        cval=vorbis_book_decode(books+info->class_book[this_class],&vb->opb);

        if(cval==-1)goto eop;
      }

      for(k=0;k<cdim;k++){
        int book=info->class_subbook[this_class][cval&(csub-1)];
        cval>>=csubbits;
        if(book>=0){
