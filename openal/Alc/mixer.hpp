...
static void LoadSamples(ALfloat *dst, const ALvoid *src, ALuint srcstep, enum FmtType srctype, ALuint samples)
{
    ...
    {
        case FmtByte:
			Load_ALbyte(dst, (const ALbyte*)src, srcstep, samples);
            break;
        case FmtShort:
			Load_ALshort(dst, (const ALshort*)  src, srcstep, samples);
            break;
        case FmtFloat:
			Load_ALfloat(dst, (const ALfloat*)src, srcstep, samples);
            break;
    }
}
    ...
    do {
        ...
        for(chan = 0;chan < NumChannels;chan++)
        {
            ...
            if(Source->SourceType == AL_STATIC)
            {
				...
				const ALubyte *Data = (const ALubyte *)ALBuffer->data;
                ALuint DataSize;
                ALuint pos;
