...
static size_t printStringChunk(Reader *stream, const RiffHdr *chnk, const char *title)
{
    ...
    {
        char *str =(char *) calloc(1, chnk->mSize+1);
...
ALboolean loadSf2(Reader *stream, ALsoundfont *soundfont, ALCcontext *context)
{
    ...
    while(list.mSize > 0 && !READERR(stream))
    {
        ...
        else if(chnk.mCode == FOURCC('i','r','o','m'))
        {
            ...
            {
                free(sfont.irom);
                sfont.irom = (ALchar *)calloc(1, chnk.mSize+1);
	...
	sfont.phdr = (PresetHeader*)calloc(sfont.phdr_size, sizeof(sfont.phdr[0]));
    for(i = 0;i < sfont.phdr_size;i++)
    ...
    sfont.pbag =(Zone*) calloc(sfont.pbag_size, sizeof(sfont.pbag[0]));
    for(i = 0;i < sfont.pbag_size;i++)
	...
	sfont.pmod = (Modulator *)calloc(sfont.pmod_size, sizeof(sfont.pmod[0]));
	...
	sfont.pgen = (Generator *) calloc(sfont.pgen_size, sizeof(sfont.pgen[0]));
	...
	sfont.inst = (InstrumentHeader*)calloc(sfont.inst_size, sizeof(sfont.inst[0]));
    for(i = 0;i < sfont.inst_size;i++)
	...
	sfont.ibag = (Zone*)calloc(sfont.ibag_size, sizeof(sfont.ibag[0]));
    for(i = 0;i < sfont.ibag_size;i++)
	...
	sfont.imod = (Modulator *)calloc(sfont.imod_size, sizeof(sfont.imod[0]));
	...
	sfont.igen = (Generator *)calloc(sfont.igen_size, sizeof(sfont.igen[0]));
	...
	sfont.shdr = (SampleHeader*)calloc(sfont.shdr_size, sizeof(sfont.shdr[0]));
	...
	presets = (ALsfpreset **)calloc(1, (soundfont->NumPresets + sfont.phdr_size - 1)*sizeof(presets[0]));
    if(!presets)
    ...
    for(i = 0;i < sfont.phdr_size-1;i++)
    {
        ...
        if(sounds_size > 0)
        {
			...
			sounds = (ALfontsound **) ExchangePtr((XchgPtr*)&presets[presets_size]->Sounds, sounds);
	...
	presets = (ALsfpreset **)ExchangePtr((XchgPtr*)&soundfont->Presets, presets);
