...
static const ALchar magicMarker00[8] = { 'M','i','n','P','H','R','0','0' };
static const ALchar magicMarker01[8] = { 'M','i','n','P','H','R','0','1' };
...
static struct Hrtf *LoadHrtf00(FILE *f, ALuint deviceRate)
{
    ...
    azCount =(ALubyte *) malloc(sizeof(azCount[0])*evCount);
	evOffset = (ALushort *)malloc(sizeof(evOffset[0])*evCount);
    ...
    if(!failed)
    {
        coeffs =(ALshort *) malloc(sizeof(coeffs[0])*irSize*irCount);
		delays = (ALubyte *)malloc(sizeof(delays[0])*irCount);
    ...
    if(!failed)
    {
		Hrtf = (struct Hrtf *)malloc(sizeof(struct Hrtf));
        if(Hrtf == NULL)
        {
            ERR("Out of memory.\n");
            failed = AL_TRUE;
