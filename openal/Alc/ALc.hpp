...
//#include "alError.h"
//#include "alMidi.h"
#include "../openal32/include/bs2b.h"
//#include "alu.h"
//
#include "compat.h"
//#include "threads.h"
...
static ALCenum UpdateDeviceParams(ALCdevice *device, const ALCint *attrList)
{
    ...
    if(device->Type == Loopback)
    {
        ...
        while(attrList[attrIdx])
        {
            if(attrList[attrIdx] == ALC_FORMAT_CHANNELS_SOFT)
            {
				...
				if (!IsValidALCChannels(val) || !ChannelsFromDevFmt((DevFmtChannels)val))
                    return ALC_INVALID_VALUE;
				schans = (DevFmtChannels)val;
                gotFmt |= GotChans;
            }
            ...
            {
                ALCint val = attrList[attrIdx + 1];
				if (!IsValidALCType(val) || !BytesFromDevFmt((DevFmtType)val))
                    return ALC_INVALID_VALUE;
                stype = (DevFmtType)val;
                gotFmt |= GotType;
            }
    ...
    if(!device->Hrtf && device->Bs2bLevel > 0 && device->Bs2bLevel <= 6)
    {
        if(!device->Bs2b)
        {
            device->Bs2b = (bs2b*)calloc(1, sizeof(*device->Bs2b));
    ...
    while(context)
    {
        ...
        {
			ALeffectslot *slot = (ALeffectslot*)context->EffectSlotMap.array[pos].value;
        ...
        {
			ALsource *source = (ALsource*) context->SourceMap.array[pos].value;
...
ALCcontext *GetContextRef(void)
{
	...
	context = (ALCcontext *)altss_get(LocalContext);
    if(context)
...
ALC_API void ALC_APIENTRY alcGetInteger64vSOFT(ALCdevice *device, ALCenum pname, ALCsizei size, ALCint64SOFT *values)
{
    ...
    {
		ivals = (ALCint*) malloc(size * sizeof(ALCint));
...
ALC_API ALCcontext* ALC_APIENTRY alcCreateContext(ALCdevice *device, const ALCint *attrList)
{
	...
	ALContext = (ALCcontext *)calloc(1, sizeof(ALCcontext) + sizeof(ALlistener));
    if(ALContext)
    {
		...
		ALContext->ActiveSources = (ALactivesource **) calloc(ALContext->MaxActiveSources,
...
{
	ALCcontext *Context = (ALCcontext*)altss_get(LocalContext);
...
{
	return (ALCcontext*)altss_get(LocalContext);
}
...
ALC_API ALCboolean ALC_APIENTRY alcMakeContextCurrent(ALCcontext *context)
{
	...
	if ((context = (ALCcontext *)altss_get(LocalContext)) != NULL)
    {
        altss_set(LocalContext, NULL);
        ALCcontext_DecRef(context);
    }
...
ALC_API ALCboolean ALC_APIENTRY alcSetThreadContext(ALCcontext *context)
{
    ...
    /* context's reference count is already incremented */
	old = (ALCcontext *)altss_get(LocalContext);
...
ALC_API ALCdevice* ALC_APIENTRY alcOpenDevice(const ALCchar *deviceName)
{
	...
	device = (ALCdevice*) al_calloc(16, sizeof(ALCdevice) + sizeof(ALeffectslot));
    if(!device)
    {
...
ALC_API ALCdevice* ALC_APIENTRY alcCaptureOpenDevice(const ALCchar *deviceName, ALCuint frequency, ALCenum format, ALCsizei samples)
{
	...
	device = (ALCdevice*)al_calloc(16, sizeof(ALCdevice));
    if(!device)
    {
        alcSetError(NULL, ALC_OUT_OF_MEMORY);
        return NULL;
...
ALC_API ALCboolean ALC_APIENTRY alcIsRenderFormatSupportedSOFT(ALCdevice *device, ALCsizei freq, ALCenum channels, ALCenum type)
{
    ...
    {
        if(IsValidALCType(type) && BytesFromDevFmt((DevFmtType)type) > 0 &&
           IsValidALCChannels(channels) && ChannelsFromDevFmt((DevFmtChannels) channels) > 0 &&
