...
ALvoid CalcNonAttnSourceParams(ALactivesource *src, const ALCcontext *ALContext)
{
...

    {
        ...
        for(c = 0;c < num_channels;c++)
        {
			...
			if (gainhf != 1.0f) src->Direct.Filters[c].ActiveTypeInt |= AF_LowPass;
			if (gainlf != 1.0f) src->Direct.Filters[c].ActiveTypeInt |= AF_HighPass;
    ...
    for(i = 0;i < NumSends;i++)
    {
        ...
        for(c = 0;c < num_channels;c++)
        {
			src->Send[i].Filters[c].ActiveTypeInt = AF_None;
			if (gainhf != 1.0f) src->Send[i].Filters[c].ActiveTypeInt |= AF_LowPass;
			if (gainlf != 1.0f) src->Send[i].Filters[c].ActiveTypeInt |= AF_HighPass;
            ALfilterState_setParams(
...
ALvoid CalcSourceParams(ALactivesource *src, const ALCcontext *ALContext)
{
...

    {
        ...
        ALfloat hfscale = ALSource->Direct.HFReference / Frequency;
        ALfloat lfscale = ALSource->Direct.LFReference / Frequency;
		src->Direct.Filters[0].ActiveTypeInt = AF_None;
		if (gainhf != 1.0f) src->Direct.Filters[0].ActiveTypeInt |= AF_LowPass;
		if (gainlf != 1.0f) src->Direct.Filters[0].ActiveTypeInt |= AF_HighPass;
        ALfilterState_setParams(
    ...
    for(i = 0;i < NumSends;i++)
    {
		...
		if (gainhf != 1.0f) src->Send[i].Filters[0].ActiveTypeInt |= AF_LowPass;
		if (gainlf != 1.0f) src->Send[i].Filters[0].ActiveTypeInt |= AF_HighPass;
        ALfilterState_setParams(
