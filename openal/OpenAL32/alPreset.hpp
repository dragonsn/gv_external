...
AL_API void AL_APIENTRY alPresetFontsoundsSOFT(ALuint id, ALsizei count, const ALuint *fsids)
{
    ...
    {
		sounds = (ALfontsound **)calloc(count, sizeof(sounds[0]));
        if(!sounds)
	...
	sounds = (ALfontsound **) ExchangePtr((XchgPtr*)&preset->Sounds, sounds);
...
ALsfpreset *NewPreset(ALCcontext *context)
{
	...
	preset = (ALsfpreset*) calloc(1, sizeof(*preset));
    if(!preset)
...
void ReleaseALPresets(ALCdevice *device)
{
    ...
    {
		ALsfpreset *temp = (ALsfpreset *)device->PresetMap.array[i].value;
