...
{
	ALsfmodulator *mod = (ALsfmodulator*) LookupUIntMapKey(&sound->ModulatorMap, id >> 2);
    if(mod) mod += id&3;
    return mod;
}
...
ALfontsound *NewFontsound(ALCcontext *context)
{
	...
	sound = (ALfontsound*)calloc(1, sizeof(*sound));
    if(!sound)
...
void ALfontsound_setPropi(ALfontsound *self, ALCcontext *context, ALenum param, ALint value)
{
    ...
    switch(param)
    {
        case AL_BUFFER:
			...
			if ((buffer = (ALbuffer *) ExchangePtr((XchgPtr*)&self->Buffer, buffer)) != NULL)
        ...
        case AL_FONTSOUND_LINK_SOFT:
			...
			if ((link = (ALfontsound *)ExchangePtr((XchgPtr*)&self->Link, link)) != NULL)
...
static ALsfmodulator *ALfontsound_getModStage(ALfontsound *self, ALsizei stage)
{
    ...
    if(!ret)
    {
		...
		ret = (ALsfmodulator *) malloc(sizeof(ALsfmodulator[4]));
        ret[0] = moddef;
        ret[1] = moddef;
        ret[2] = moddef;
        ret[3] = moddef;
...
void ReleaseALFontsounds(ALCdevice *device)
{
    ...
    {
		ALfontsound *temp = (ALfontsound*) device->FontsoundMap.array[i].value;
