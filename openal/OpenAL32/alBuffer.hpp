  ...
  ALenum channels, ALenum type, const ALvoid *data)
{
    ...
    align = albuf->UnpackAlign;
	if (SanitizeAlignment((UserFmtType)type, &align) == AL_FALSE)
        SET_ERROR_AND_GOTO(context, AL_INVALID_VALUE, done);
    if((samples%align) != 0)
        SET_ERROR_AND_GOTO(context, AL_INVALID_VALUE, done);

    err = LoadData(albuf, samplerate, internalformat, samples,
		(UserFmtChannels)channels, (UserFmtType)type, data, align, AL_FALSE);
    if(err != AL_NO_ERROR)
        SET_ERROR_AND_GOTO(context, err, done);
  ...
  ALenum channels, ALenum type, const ALvoid *data)
{
    ...
    ConvertData((char*)albuf->data+offset, (enum UserFmtType)albuf->FmtType,
		data, (UserFmtType)type, ChannelsFromFmt(albuf->FmtChannels), samples, align);
    WriteUnlock(&albuf->lock);
  ...
  ALenum channels, ALenum type, ALvoid *data)
{
    ...
    /* offset -> byte offset */
    offset *= FrameSizeFromFmt(albuf->FmtChannels, albuf->FmtType);
	ConvertData(data, (UserFmtType) type, (char*)albuf->data + offset, (enum UserFmtType)albuf->FmtType,
...
ALbuffer *NewBuffer(ALCcontext *context)
{
	...
	buffer = (ALbuffer*)calloc(1, sizeof(ALbuffer));
    if(!buffer)
...
ALvoid ReleaseALBuffers(ALCdevice *device)
{
    ...
    {
		ALbuffer *temp = (ALbuffer*)device->BufferMap.array[i].value;
