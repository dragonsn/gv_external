...
static void Convert_##T1##_##T2(T1 *dst, const void *_src, ALuint numchans,   \
                                ALuint len, ALsizei UNUSED(align))            \
{                                                                             \
    ALuint i, j;  const T2 * src=(const T2 *) _src ;                          \
    for(i = 0;i < len;i++)                                                    \
    {                                                                         \
...
{                                                                             \
    ...
    {                                                                         \
        case UserFmtByte:                                                     \
            Convert_##T##_ALbyte(dst, (ALbyte*)src, numchans, len, align);             \
            break;                                                            \
        case UserFmtUByte:                                                    \
            Convert_##T##_ALubyte(dst, (ALubyte*)src, numchans, len, align);            \
            break;                                                            \
        case UserFmtShort:                                                    \
            Convert_##T##_ALshort(dst, (ALshort*)src, numchans, len, align);            \
            break;                                                            \
        case UserFmtUShort:                                                   \
            Convert_##T##_ALushort(dst, (ALushort*)src, numchans, len, align);           \
            break;                                                            \
        case UserFmtInt:                                                      \
            Convert_##T##_ALint(dst, (ALint*)src, numchans, len, align);              \
            break;                                                            \
        case UserFmtUInt:                                                     \
            Convert_##T##_ALuint(dst, (ALuint*)src, numchans, len, align);             \
            break;                                                            \
        case UserFmtFloat:                                                    \
            Convert_##T##_ALfloat(dst, (ALfloat*)src, numchans, len, align);            \
            break;                                                            \
        case UserFmtDouble:                                                   \
            Convert_##T##_ALdouble(dst,(ALdouble*) src, numchans, len, align);           \
            break;                                                            \
        case UserFmtMulaw:                                                    \
            Convert_##T##_ALmulaw(dst,(ALmulaw*) src, numchans, len, align);            \
            break;                                                            \
        case UserFmtAlaw:                                                     \
            Convert_##T##_ALalaw(dst, (ALalaw*)src, numchans, len, align);             \
            break;                                                            \
        case UserFmtIMA4:                                                     \
            Convert_##T##_ALima4(dst,(ALima4*) src, numchans, len, align);             \
            break;                                                            \
        case UserFmtMSADPCM:                                                  \
            Convert_##T##_ALmsadpcm(dst,(ALmsadpcm*) src, numchans, len, align);          \
            break;                                                            \
        case UserFmtByte3:                                                    \
            Convert_##T##_ALbyte3(dst, (ALbyte3*)src, numchans, len, align);            \
            break;                                                            \
        case UserFmtUByte3:                                                   \
            Convert_##T##_ALubyte3(dst,(ALubyte3*) src, numchans, len, align);           \
            break;                                                            \
    }                                                                         \
}																				\
	static void Convert_##T(void *dst, const ALvoid *src, enum UserFmtType srcType,  \
                        ALsizei numchans, ALsizei len, ALsizei align)				{\
						return Convert_##T((T*)dst, src,srcType,numchans,len,align);	\
	}
