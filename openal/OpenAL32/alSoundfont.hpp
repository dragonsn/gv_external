...
AL_API void AL_APIENTRY alGenSoundfontsSOFT(ALsizei n, ALuint *ids)
{
    ...
    for(cur = 0;cur < n;cur++)
    {
		ALsoundfont *sfont = (ALsoundfont *) calloc(1, sizeof(ALsoundfont));
        if(!sfont)
        {
...
AL_API void AL_APIENTRY alSoundfontPresetsSOFT(ALuint id, ALsizei count, const ALuint *pids)
{
    ...
    {
		presets = (ALsfpreset **)calloc(count, sizeof(presets[0]));
        if(!presets)
        {
            WriteUnlock(&sfont->Lock);
            SET_ERROR_AND_GOTO(context, AL_OUT_OF_MEMORY, done);
	...
	presets = (ALsfpreset **) ExchangePtr((XchgPtr*)&sfont->Presets, presets);
...
ALsoundfont *ALsoundfont_getDefSoundfont(ALCcontext *context)
{
	...
	device->DefaultSfont = (ALsoundfont *)calloc(1, sizeof(device->DefaultSfont[0]));
...
void ALsoundfont_deleteSoundfont(ALsoundfont *self, ALCdevice *device)
{
    ...
    VECTOR_INIT(buffers);
	presets = (ALsfpreset ** )ExchangePtr((XchgPtr*)&self->Presets, NULL);
    ...
    for(i = 0;i < num_presets;i++)
    {
		...
		sounds = (ALfontsound **)ExchangePtr((XchgPtr*)&preset->Sounds, NULL);
...
void ReleaseALSoundfonts(ALCdevice *device)
{
    ...
    {
		ALsoundfont *temp = (ALsoundfont *)device->SfontMap.array[i].value;
