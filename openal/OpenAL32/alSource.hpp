...
static ALboolean SetSourcefv(ALsource *Source, ALCcontext *Context, int prop, const ALfloat *values){
	return SetSourcefv(Source, Context, (SrcFloatProp)prop, values);
};
static ALboolean SetSourceiv(ALsource *Source, ALCcontext *Context, int prop, const ALint *values){
	return SetSourceiv(Source, Context, (SrcIntProp)prop, values);
};
static ALboolean SetSourcei64v(ALsource *Source, ALCcontext *Context, int prop, const ALint64SOFT *values){
	return SetSourcei64v(Source, Context, (SrcIntProp)prop, values);
};
...
static ALboolean GetSourcedv(ALsource *Source, ALCcontext *Context, int prop, ALdouble *values)
{
	return GetSourcedv(Source, Context, (SrcFloatProp)prop, values);
};

static ALboolean GetSourceiv(ALsource *Source, ALCcontext *Context, int prop, ALint *values)
{
	return GetSourceiv(Source, Context, (SrcIntProp)prop, values);
};

static ALboolean GetSourcei64v(ALsource *Source, ALCcontext *Context, int prop, ALint64 *values)
{
	return GetSourcei64v(Source, Context, (SrcIntProp)prop, values);
};
    ...
    switch(prop)
    {
        ...
        case AL_BUFFER:
            ...
            if(buffer != NULL)
            {
				...
				newlist = (ALbufferlistitem *)malloc(sizeof(ALbufferlistitem));
        ...
        case AL_DISTANCE_MODEL:
			...
			Source->DistanceModel = (DistanceModel)*values;
        ...
        case AL_AUXILIARY_SEND_FILTER:
			...
			slot = (ALeffectslot*) ExchangePtr((XchgPtr*)&Source->Send[values[1]].Slot, slot);
...
AL_API ALvoid AL_APIENTRY alGenSources(ALsizei n, ALuint *sources)
{
    ...
    for(cur = 0;cur < n;cur++)
    {
		ALsource *source = (ALsource*)al_calloc(16, sizeof(ALsource));
        if(!source)
        {
...
AL_API ALvoid AL_APIENTRY alSourcePlayv(ALsizei n, const ALuint *sources)
{
    ...
    while(n > context->MaxActiveSources-context->ActiveSourceCount)
    {
        ...
        if(newcount > 0)
			temp = (ALactivesource **)realloc(context->ActiveSources,
...
AL_API ALvoid AL_APIENTRY alSourceQueueBuffers(ALuint src, ALsizei nb, const ALuint *buffers)
{
    ...
    for(i = 0;i < nb;i++)
    {
        ...
        if(!BufferListStart)
        {
			BufferListStart = (ALbufferlistitem *)malloc(sizeof(ALbufferlistitem));
        ...
        {
			BufferList->next = (ALbufferlistitem *)malloc(sizeof(ALbufferlistitem));
...
ALvoid SetSourceState(ALsource *Source, ALCcontext *Context, ALenum state)
{
    ...
    if(state == AL_PLAYING)
    {
        ...
        if(src == NULL)
        {
            ...
            if(src == NULL)
            {
				src = (ALactivesource *)al_malloc(16, sizeof(src[0]));
...
ALvoid ReleaseALSources(ALCcontext *Context)
{
    ...
    {
		ALsource *temp = (ALsource*)Context->SourceMap.array[pos].value;
