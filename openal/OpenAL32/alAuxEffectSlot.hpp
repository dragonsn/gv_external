...
#include "include/alAuxEffectSlot.h"
...
{
	ALeffectStateFactory* (*getFactory)(void) = (ALeffectStateFactory*(*)(void) )LookupUIntMapKey(&EffectStateFactoryMap, type);
    if(getFactory != NULL)
        return getFactory();
    return NULL;
}
...
AL_API ALvoid AL_APIENTRY alGenAuxiliaryEffectSlots(ALsizei n, ALuint *effectslots)
{
    ...
    for(cur = 0;cur < n;cur++)
    {
		ALeffectslot *slot = (ALeffectslot *)  al_calloc(16, sizeof(ALeffectslot));
...
ALenum InitializeEffect(ALCdevice *Device, ALeffectslot *EffectSlot, ALeffect *effect)
{
    ...
    if(newtype != EffectSlot->EffectType)
    {
		...
		State = (ALeffectState*) ExchangePtr((XchgPtr*)&EffectSlot->EffectState, State);
        if(!effect)
        {
...
ALvoid ReleaseALAuxiliaryEffectSlots(ALCcontext *Context)
{
    ...
    {
		ALeffectslot *temp = (ALeffectslot *)Context->EffectSlotMap.array[pos].value;
