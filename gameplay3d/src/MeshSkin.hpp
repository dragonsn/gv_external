...
Vector4* MeshSkin::getMatrixPalette() const
{
    ...
    for (size_t i = 0, count = _joints.size(); i < count; i++)
    {
        if (_joints[i]) _joints[i]->updateJointMatrix(getBindShape(), &_matrixPalette[i * PALETTE_ROWS]);
    }
...
void MeshSkin::setMatrixPalette( const float *  matrix)
{
	 GP_ASSERT(_matrixPalette);
	 memcpy(_matrixPalette, matrix, getMatrixPaletteSize()*16);
