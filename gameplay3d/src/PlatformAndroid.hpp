...
static bool initEGL()
{
	...
	GVM_DEBUG_OUT("screen size :"<<__width<<","<<__height);
...
{
	GVM_DEBUG_OUT("XXX Platform::create");
...
int Platform::startupMessagePump()
{
	...
	GVM_DEBUG_OUT("XXX Platform::startupMessagePump");
...
{
	GV_PROFILE_EVENT(Platform_tickMessagePump,0);
	...
	while (true)
	{
		...
		{
			GV_PROFILE_EVENT(game__frame,0);
			_game->frame();
