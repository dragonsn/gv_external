...
{
    //should allow user create game later
...
int Game::run()
{
	...
	_ui_scale_x=_ui_scale_y=1.0f;

	if (config)
	{
		_design_width = config->getInt("design_width");
		_design_height = config->getInt("design_height");
		if (!_design_width)
			_design_width = config->getInt("width");
		if (!_design_height) 
			_design_height = config->getInt("height");
		if (_design_width){
			_ui_scale_x=(float)_width/(float)_design_width;
		}
		if (_design_height){
			_ui_scale_y=(float)_height/(float)_design_height;
		}
	}
...
{
	GV_PROFILE_EVENT(gv_gameplay___clear, 0);
	GLbitfield bits = 0;
