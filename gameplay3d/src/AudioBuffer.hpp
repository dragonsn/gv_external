...
AudioBuffer* AudioBuffer::create(const char* path)
{
    ...
    if (AL_LAST_ERROR())
    {
        GP_ERROR("Failed to create OpenAL buffer; alGenBuffers error: "<< AL_LAST_ERROR());
    ...
    if (memcmp(header, "RIFF", 4) == 0)
    {
        ...
        {
            GP_ERROR("Invalid wave file: "<< path);
            goto cleanup;
        }
    }
...
bool AudioBuffer::loadWav(Stream* stream, ALuint buffer)
{
    ...
    {
        GP_ERROR("Incompatible wave file format: "<< channels<< bits);
        return false;
    }
    ...
    while (true)
    {
        ...
        else
        {
            ...
            if (stream->read(data, 1, 4) != 4)
            {
                GP_ERROR("Failed to read size of  chunk from wave file. size:"<< chunk);
                return false;
            }
