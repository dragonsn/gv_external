...
public:
	...
	/**
	* controller callback on gamepad events.  Override to receive Gamepad::CONNECTED_EVENT 
	* and Gamepad::DISCONNECTED_EVENT, and store the Gamepad* in order to poll it from update().
	*
	* @param evt The control event that occurred.
	* @param gamepad The gamepad that generated the event.
	*/
	virtual void controlEvent(class Control* control, int evt){};
    /**
...
public:
	...
	int _design_width;                        // The game's display width in design.
	int _design_height;                       // The game's display height int design. 
	float _ui_scale_x;
	float _ui_scale_y;
};
