...
{
	GV_PROFILE_EVENT(GP_Terrain__create, 0);
	// Terrain properties
   ...
   // float halfWidth = (width - 1) * 0.5f;
    ...
    for (unsigned int z = 0; z < height-1; z = z2, ++row)
    {
        ...
        for (unsigned int x = 0; x < width-1; x = x2, ++column)
        {
            ...
            // Create this patch
            TerrainPatch* patch = TerrainPatch::create(terrain, row, column, heightfield->getArray(), width, height, x1, z1, x2, z2, 0/*-halfWidth*/, 0/*-halfHeight*/, maxStep, skirtScale);
