...
{
	GV_PROFILE_EVENT(Effect__createFromSource, 0);
	GP_ASSERT(vshSource);
    GP_ASSERT(fshSource);
	...
	GVM_DEBUG_OUT("xxx compile vertex shader " << (vshPath == NULL ? vshSource : vshPath));
    if (success != GL_TRUE)
    {
		...
		GP_WARN(shaderSource[2]);
		gv::gv_save_string_to_file("shader.txt", gv::gv_string_tmp( shaderSource[2]));
	...
	GVM_DEBUG_OUT("xxx compile fragment shader " << (fshPath == NULL ? fshSource : fshPath));
    if (success != GL_TRUE)
    {
    ...
    GL_ASSERT( glLinkProgram(program) );
		...
		GP_ASSERT(0 && "try to use the nv adater for this ");
	}
