...
Texture* Texture::create(const char* path, bool generateMipmaps,bool load_default)
{
    GP_ASSERT(path);
	...
	if (load_default) return  create("tex/DEFAULT.DDS", true, false);
	return NULL;
}
...
Texture* Texture::createCompressedDDS(const char* path)
{
    ...
    if (stream.get() == NULL || !stream->canRead())
    {
        GP_WARN("Failed to open file "<< path);
        return NULL;
    }
    ...
    {
        GP_ERROR("Failed to read DDS file : invalid DDS magic number."<< path);
        return NULL;
    }
    ...
    {
        GP_ERROR("Failed to read header for DDS file "<< path);
        return NULL;
    }
