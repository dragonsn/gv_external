...
Form* Form::create(const char* url)
{
	...
	GV_PROFILE_EVENT(Form_create, 0);
	...
	form->_directDraw=formProperties->getBool("direct_draw");
...
void Form::setSize(float width, float height)
{
        ...
        (width != _bounds.width || height != _bounds.height))
    {
		...
		{
			//use viewport to offset .
			Matrix::createOrthographicOffCenter(0, width, height, 0, 0, 1, &_projectionMatrix);
			//Matrix::createOrthographicOffCenter(0,  Game::getInstance()->getWidth(),  Game::getInstance()->getHeight(), 0, 0, 1, &_projectionMatrix);
		}
...
void Form::draw()
{
	...
	GV_PROFILE_EVENT(gp_form_draw, 0);
	if (_directDraw)
	{
		Game* game = Game::getInstance();
