    ...
    case Mesh::TRIANGLES:
        {
            ...
            {
				void * poffset = (char*)0 + i*indexSize;
                GL_ASSERT( glDrawElements(GL_LINE_LOOP, 3, part->getIndexFormat(), poffset));
            }
        }
        return true;
    ...
    case Mesh::TRIANGLE_STRIP:
        {
            ...
            {
				void * poffset = (char*)0 + ((i - 2)*indexSize);
                GL_ASSERT( glDrawElements(GL_LINE_LOOP, 3, part->getIndexFormat(), poffset));
            }
        }
        return true;
...
{
    GP_ASSERT(_mesh);
	GV_PROFILE_EVENT(gp_model_draw, (gv_int_ptr)this);
    ...
    if (partCount == 0)
    {
        ...
        if (_material)
        {
            ...
            for (unsigned int i = 0; i < passCount; ++i)
            {
                ...
                {
					GV_PROFILE_EVENT(glDrawArrays, 0);
        ...
        for (unsigned int i = 0; i < partCount; ++i)
        {
            ...
            if (material)
            {
                ...
                for (unsigned int j = 0; j < passCount; ++j)
                {
                    ...
                    {
						GV_PROFILE_EVENT(glDrawElements, 0);
