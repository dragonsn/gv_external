...
void Control::initialize(Theme::Style* style, Properties* properties)
{
	...
	Vector2  scale(Game::getInstance()->_ui_scale_x,Game::getInstance()->_ui_scale_y); 
	position*=scale; 
	size*=scale;
...
void Control::notifyListeners(Control::Listener::EventType eventType)
{
	...
	Game::getInstance()->controlEvent(this,eventType);
#if GP_WITH_LUA
...
void Control::overrideThemedProperties(Properties* properties, unsigned char states)
{
    ...
    {
		int fontSize=properties->getInt("fontSize");
		fontSize=(int )((float)fontSize* Game::getInstance()->_ui_scale_y);
        setFontSize(fontSize, states);
    }
