...
namespace DollarRecognizer
{
	...
	void GeometricRecognizer::loadTemplates()
	{
		...
		addTemplate("circle_anti_clock_wise", samples.getGestureCircle());
		Path2D p= samples.getGestureCircle();
		 std::reverse(p.begin(), p.end());
		 addTemplate("circle_clock_wise", p);
		...
		addTemplate("left", samples.getGestureLeft());
		addTemplate("right", samples.getGestureRight());
	...
	Path2D GeometricRecognizer::resample(Path2D points)
	{
		...
		if (newPoints.size() ==(unsigned) (numPointsInGesture - 1) )
		{
